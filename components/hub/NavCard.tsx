...
export default function NavCard({ item }: Props) {
  return (
    <Link
      key={`nav-${item.name}`}
      href={item.href}
      className="group relative flex max-w-[10rem] scale-100 flex-col gap-y-1 self-stretch rounded-md border border-cyan-900 bg-cyan-800/80 px-2 py-1.5 font-light text-white duration-100 hover:scale-105 hover:border-slate-700 hover:bg-cyan-600/60 dark:border-slate-200 dark:bg-slate-800/60 dark:text-white dark:hover:border-white dark:hover:bg-violet-400/50 md:max-w-xs md:border-2 md:px-4 md:py-4"
    >
